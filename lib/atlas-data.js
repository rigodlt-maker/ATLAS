...
// ============================================================
// Mapa histórico (nuevo)
// ============================================================

// Devuelve todos los nodos que tienen un claim de coordenadas
// (value_type = 'geographic_reference', v3 §5.1) con geometry tipo Point.
export async function getMapNodes() {
  const pool = getPool();
  const res = await pool.query(`
    SELECT c.value, c.certainty, c.claim_status,
           n.slug, n.type, n.node_kind,
           COALESCE(t.content, n.slug) AS name
    FROM claims c
    JOIN nodes n ON n.id = c.subject_id AND c.subject_type = 'node'
    LEFT JOIN translations t
      ON t.entity_type = 'node' AND t.entity_id = n.id AND t.field = 'name' AND t.locale = 'es'
    WHERE c.is_current = true
      AND c.value_type = 'geographic_reference'
      AND n.is_current = true
  `);

  return res.rows
    .filter((r) => r.value?.geometry?.coordinates)
    .map((r) => ({
      slug: r.slug,
      name: r.name,
      type: r.type,
      node_kind: r.node_kind,
      lng: r.value.geometry.coordinates[0],
      lat: r.value.geometry.coordinates[1],
